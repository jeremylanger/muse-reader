...
`,sd=new DOMParser,_c=sd.parseFromString(Nc.replaceAll(/<\?xml (.*)\?>/g,""),"text/xml"),ws=Array.from(_c.body.children),ad=e=>sd.parseFromString(e.replaceAll(/<\?xml (.*)\?>/g,""),"text/xml"),zc=e=>{var s;const n=(s=ad(e).querySelector("rootfile"))==null?void 0:s.getAttribute("full-path");if(!n)throw new Error("No full-path attribute found in container.xml");return n},od=e=>new Promise((t,n)=>{const s=new FileReader;s.onload=a=>{var i;const o=(i=a==null?void 0:a.target)==null?void 0:i.result;if(!o){console.log("File is empty"),n(new Error("File is empty"));return}t(o)},s.onerror=n,s.readAsText(e)}),id=async e=>fetch(e).then(t=>{if(!t.ok)throw new Error("Network response was not ok");return t.blob()}).catch(t=>{throw console.error(t),t}),Wc=async()=>{const e=await id("/assets/dune-v2/META-INF/container.xml");try{const t=await od(e);return zc(t)}catch(t){console.error("Error reading file:",t)}},Mc=async e=>{var n;const t=await id(`/assets/dune-v2/${e}`);try{const s=await od(t),o=(n=ad(s).querySelector("manifest"))==null?void 0:n.children;if(!o)throw new Error(`No manifest found in ${e}`);return o}catch(s){console.error("Error reading file:",s)}},Yc=async()=>{const e=await Wc();if(console.log({rootFilePath:e}),!e){console.error("No root file path found");return}const t=await Mc(e);if(!t){console.error("No manifest found");return}const n=Array.from(t).filter(o=>o.getAttribute("media-type")==="application/xhtml+xml").map(o=>o.getAttribute("href")),s=e.split("/").slice(0,-1).join("/"),a=n.map(o=>`${s}/${o}`);console.log(a)};Yc();function Oc(){const e=+(localStorage.getItem("sentenceIndex")||0),[t,n]=et.useState(e),s=et.useCallback(p=>{(p.key==="ArrowRight"||p.key===" ")&&n(w=>Math.min(ws.length-1,w+1)),p.key==="ArrowLeft"&&n(w=>Math.max(0,w-1))},[]);et.useEffect(()=>(document.addEventListener("keydown",s),()=>document.removeEventListener("keydown",s)),[s]),et.useEffect(()=>{localStorage.setItem("sentenceIndex",t.toString())},[t]);const a=p=>{const v=p.length*40,k=p.endsWith(",")?200:0,B=p.endsWith(";")||p.endsWith(".")||p.endsWith('."')||p.endsWith(".’")||p.endsWith("!")||p.endsWith('!"')||p.endsWith("!’")||p.endsWith("?")||p.endsWith('?"')||p.endsWith("?’")?350:0,u=p.endsWith("…")?1e3:0;return 150+v+k+B+u},o=p=>p.startsWith("<em>"),i=p=>p.endsWith("</em>")||p.endsWith("</em>,")||p.endsWith("</em>."),r=p=>p.children.length>0?Array.from(p.children).map(v=>r(v)).join("<br> "):l(p.innerHTML),l=p=>p.replaceAll("…",". . .").replaceAll(/<em xmlns=["'].*["']>/g,"<em>").replaceAll(/<a xmlns=["'].*["'] id=["'].*["']><\/a>/g,"");let d=0,m=!1;const f=r(ws[t]).split(" ").filter(p=>p!=="");return Ce.jsxs("div",{className:"absolute top-0 left-0 right-0 bottom-0 bg-cover bg-[url('/dune-wallpaper.jpg')] bg-center text-center text-white font-dm-serif p-8 text-2xl leading-normal sm:text-5xl sm:leading-tight",children:[Ce.jsx("div",{className:"flex content-center h-full",children:Ce.jsx("div",{className:`max-w-[800px] m-auto select-none node-${ws[t].nodeName}`,children:f.map((p,w)=>{let v=p;o(p)&&(m=!0),m&&(v=`<em>${p}</em>`),i(p)&&(m=!1);const k=w===0?"":f[w-1];return d+=a(k),Ce.jsx(Bc,{delay:d,word:v},`${t}-${w}-${p}`)})})}),Ce.jsx("div",{className:"fixed bottom-0 left-0 right-0 h-1",children:Ce.jsx("div",{className:"h-full bg-white opacity-50 transition-all rounded",style:{width:`${t/(ws.length-1)*100}%`}})})]})}Ka.createRoot(document.getElementById("root")).render(Ce.jsx(Oc,{}));
